 ...
 */
abstract contract MinaMultisig is State {
     ...
     */
    event Confirmation(
        HashedMinaPublicKey indexed sender,
    ...
    function removeValidator(
        ...
        for (uint256 i = 0; i < validators.length - 1; i++) {
            if (
                validators[i].x == validator.x && validators[i].y == validator.y
            ) {
        ...
        validatorDoesNotExist(hashPublicKey(newValidator))
    {
        ...
        for (uint256 i = 0; i < validators.length; i++) {
            if (
                validators[i].x == oldValidator.x &&
                validators[i].y == oldValidator.y
            ) {
    ...
    function getConfirmations(
        ...
        MinaPublicKey[] memory confirmationsTemp = new MinaPublicKey[](
            validators.length
        );
    ...
    function getTransactionIds(
        ...
        for (uint256 i = 0; i < count; i++)
            transactionIds_[i] = transactionIdsTemp[i];
    }
